...
#[test]
fn xresizewindow_starts_interactive_resize() {
    let mut f = Fixture::new();
    let mut connection = Connection::new(&f.display);
    ...
    connection
        .send_and_check_request(&x::ConfigureWindow {
            window,
            value_list: &[x::ConfigWindow::Width(40), x::ConfigWindow::Height(50)],
        })
        .unwrap();
...
#[test]
fn xresizewindow_without_pointer_grab_is_not_interactive() {
    let mut f = Fixture::new();
    let mut connection = Connection::new(&f.display);
    ...
    f.wait_and_dispatch();
    let data = f.testwl.get_surface_data(surface).unwrap();
    assert_eq!(data.resizing, None);
}

#[test]
fn xresizewindow_after_pointer_release_is_not_interactive() {
    let mut f = Fixture::new();
    let mut connection = Connection::new(&f.display);
    ...
    ptr.frame();
    ptr.button(10, 20, BTN_LEFT, wl_pointer::ButtonState::Pressed);
    ptr.button(11, 21, BTN_LEFT, wl_pointer::ButtonState::Released);
    ptr.frame();
    f.testwl.dispatch();
